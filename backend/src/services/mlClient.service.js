...
export const mlClient = {
  ...
  async predictVideo(fileBuffer, originalName, mimeType) {
    try {
      const formData = new FormData();
      // Node.js >= 18 supports native FormData and Blob
      const blob = new Blob([fileBuffer], { type: mimeType });
      ...
      const url = `${config.mlModelUrl}/api/v1/predict?type=video`;
      ...
      console.error('Error communicating with ML server (video):', err.message);
      if (err.response) {
        throw new Error(`ML Server Error (${err.response.status}): ${JSON.stringify(err.response.data)}`);
      }
      throw new Error(`ML server connection failed: ${err.message}`);
